...
import { LogOut, User, Mail, Shield, Bell, CircleHelp, Moon, Sun } from "lucide-react-native";
        ...
        <SafeAreaView className="flex-1 bg-gray-50 dark:bg-slate-950">
            <ScrollView className="flex-1 px-6 pt-8">
                <View className="items-center mb-10">
                    ...
                    <Text className="text-gray-500 dark:text-slate-400 mt-1">{user?.email}</Text>
                </View>
                ...
                <View className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden mb-8">
                    <View className="p-4 border-b border-gray-50 dark:border-slate-800 flex-row items-center">
                        ...
                        <View className="ml-3">
                            <Text className="text-xs text-gray-400 dark:text-slate-500 font-medium uppercase tracking-wider">Email Address</Text>
                            <Text className="text-gray-900 dark:text-white font-medium">{user?.email}</Text>
                        </View>
                    </View>
                </View>

                <Text className="text-sm font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-1">Settings</Text>
                <View className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden mb-8">
                    {menuItems.map((item, index) => (
                        ...
                        >
                            <View className="flex-row items-center">
                                ...
                                <Text className="ml-3 text-gray-700 dark:text-slate-200 font-medium">{item.label}</Text>
                            </View>
                            {item.label === "Dark Mode" ? (
                                <View className={`w-10 h-6 rounded-full px-1 justify-center ${item.value ? 'bg-indigo-600' : 'bg-gray-200'}`}>
                                    <View className={`w-4 h-4 rounded-full bg-white shadow-sm ${item.value ? 'translate-x-4' : 'translate-x-0'}`} />
                                </View>
                            ) : (
                ...
                <View className="items-center mb-10">
                    <Text className="text-gray-300 text-xs font-medium">Flashcard App v1.0.0</Text>
                </View>
            </ScrollView>
        </SafeAreaView>
    );
